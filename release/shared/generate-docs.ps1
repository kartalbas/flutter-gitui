...
function New-Pdf {
    ...
    try {
        & pandoc $readmePath -o $pdfPath --pdf-engine=xelatex -V geometry:margin=1in 2>&1 | Out-Null
    ...
    try {
        & pandoc $readmePath -o $pdfPath --pdf-engine=pdflatex -V geometry:margin=1in 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  [OK] README.pdf generated" -ForegroundColor Green
            return $true
        }
