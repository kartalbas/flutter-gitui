...
class ConfigNotifier extends StateNotifier<AppConfig> {
  ...
  ConfigNotifier.withConfig(this._ref, AppConfig config) : super(config) {
    ...
    _loadFailed = false;
    Logger.info(
      '[CONFIG] Using pre-loaded configuration (gitPath=${config.git.executablePath ?? "null"})',
    );
    ...
    Future(() async {
      ...
      Logger.info(
        '[CONFIG] Pre-loaded config initialization completed (_configLoadingStateProvider set to false)',
      );
    });
  }
  ...
  Future<void> _loadConfig() async {
    ...
    try {
      ...
      Logger.debug(
        'After ConfigService.load() - repositories count: ${config.workspace.repositories.length}',
      );
      Logger.debug(
        'After ConfigService.load() - workspaces count: ${config.workspace.workspaces.length}',
      );
      Logger.debug(
        '[CONFIG] Git executable path from config: ${config.git.executablePath ?? "null"}',
      );

      // Validate and update tool versions on startup
      config = await _validateToolVersions(config);
      Logger.debug(
        'After _validateToolVersions() - repositories count: ${config.workspace.repositories.length}',
      );
      Logger.debug(
        'After _validateToolVersions() - workspaces count: ${config.workspace.workspaces.length}',
      );
      ...
      Logger.debug(
        'After _validateWorkspace() - repositories count: ${config.workspace.repositories.length}',
      );
      Logger.debug(
        'After _validateWorkspace() - workspaces count: ${config.workspace.workspaces.length}',
      );
      ...
      Logger.info(
        '[CONFIG] Configuration loaded successfully (gitPath=${config.git.executablePath ?? "null"})',
      );
    } catch (e, stack) {
      ...
      _ref.read(_configLoadingStateProvider.notifier).state = false;
      Logger.info(
        '[CONFIG] Loading configuration completed (_isLoading=false)',
      );
    }
  }
  ...
  Future<void> _validatePreloadedConfig() async {
    try {
      ...
      state = config;
      Logger.info(
        '[CONFIG] Pre-loaded configuration validated (gitPathInvalid=$_gitPathInvalid)',
      );
    } catch (e, stack) {
  ...
  ({AppConfig config, bool needsSave}) _validateWorkspace(AppConfig config) {
    ...
    final exists = config.workspace.repositories.any(
      (r) => r.path == currentRepo,
    );

    if (!exists) {
      Logger.warning(
        'Current repository "$currentRepo" not found in workspace - clearing selection',
      );
      // Clear the orphaned current repository
      final updatedConfig = config.copyWith(
        workspace: config.workspace.copyWith(currentRepository: null),
      );
  ...
  Future<AppConfig> _validateToolVersions(AppConfig config) async {
    ...
    if (config.git.executablePath != null &&
        config.git.executablePath!.isNotEmpty) {
      try {
        final version = await VersionDetector.detectVersion(
          config.git.executablePath!,
        );
        if (version != null && version != config.git.gitVersion) {
          Logger.config(
            'Updated git version: ${config.git.gitVersion} → $version',
          );
    ...
    if (config.tools.textEditor != null &&
        config.tools.textEditor!.isNotEmpty) {
      try {
        final version = await VersionDetector.detectVersion(
          config.tools.textEditor!,
        );
        if (version != null && version != config.tools.textEditorVersion) {
          Logger.config(
            'Updated text editor version: ${config.tools.textEditorVersion} → $version',
          );
          updatedConfig = updatedConfig.copyWith(
    ...
    if (config.tools.diffToolPath != null &&
        config.tools.diffToolPath!.isNotEmpty) {
      try {
        final version = await VersionDetector.detectVersion(
          config.tools.diffToolPath!,
        );
        if (version != null && version != config.tools.diffToolVersion) {
          Logger.config(
            'Updated diff tool version: ${config.tools.diffToolVersion} → $version',
          );
          updatedConfig = updatedConfig.copyWith(
    ...
    if (config.tools.mergeToolPath != null &&
        config.tools.mergeToolPath!.isNotEmpty) {
      try {
        final version = await VersionDetector.detectVersion(
          config.tools.mergeToolPath!,
        );
        if (version != null && version != config.tools.mergeToolVersion) {
          Logger.config(
            'Updated merge tool version: ${config.tools.mergeToolVersion} → $version',
          );
          updatedConfig = updatedConfig.copyWith(
    ...
    if (config.tools.customDiffToolPath != null &&
        config.tools.customDiffToolPath!.isNotEmpty) {
      try {
        final version = await VersionDetector.detectVersion(
          config.tools.customDiffToolPath!,
        );
        if (version != null && version != config.tools.customDiffToolVersion) {
          Logger.config(
            'Updated custom diff tool version: ${config.tools.customDiffToolVersion} → $version',
          );
          updatedConfig = updatedConfig.copyWith(
    ...
    if (config.tools.customMergeToolPath != null &&
        config.tools.customMergeToolPath!.isNotEmpty) {
      try {
        final version = await VersionDetector.detectVersion(
          config.tools.customMergeToolPath!,
        );
        if (version != null && version != config.tools.customMergeToolVersion) {
          Logger.config(
            'Updated custom merge tool version: ${config.tools.customMergeToolVersion} → $version',
          );
          updatedConfig = updatedConfig.copyWith(
            tools: updatedConfig.tools.copyWith(
  ...
  Future<void> _saveConfig() async {
    try {
      ...
      Logger.debug(
        'Current state.git.executablePath = ${state.git.executablePath}',
      );
      Logger.debug(
        'Current state.tools.textEditor = ${state.tools.textEditor}',
      );
      Logger.debug(
        'Current state.tools.customDiffToolPath = ${state.tools.customDiffToolPath}',
      );
      Logger.debug(
        'Current state.tools.customMergeToolPath = ${state.tools.customMergeToolPath}',
      );
      Logger.debug(
        'Current state.git.defaultUserName = ${state.git.defaultUserName}',
      );
      Logger.debug(
        'Current state.git.defaultUserEmail = ${state.git.defaultUserEmail}',
      );
  ...
  Future<void> setGitExecutablePath(String? path, {String? version}) async {
    ...
    if (path == null) {
      ...
      state = state.copyWith(
        git: state.git.copyWith(executablePath: path, gitVersion: version),
      );
    }
    Logger.debug(
      'After setting state, git.executablePath = ${state.git.executablePath}',
    );
    Logger.debug(
      'After setting state, git.gitVersion = ${state.git.gitVersion}',
    );
    await _saveConfig();
  }
  ...
  Future<void> setDefaultUserName(String? name) async {
    ...
    Logger.debug(
      'After setting state, git.defaultUserName = ${state.git.defaultUserName}',
    );
    await _saveConfig();
  }
  ...
  Future<void> setDefaultUserEmail(String? email) async {
    ...
    Logger.debug(
      'After setting state, git.defaultUserEmail = ${state.git.defaultUserEmail}',
    );
    await _saveConfig();
  }
  ...
  Future<void> setTextEditor(String? editor, {String? version}) async {
    ...
    Logger.debug(
      'After setting state, tools.textEditor = ${state.tools.textEditor}',
    );
    Logger.debug(
      'After setting state, tools.textEditorVersion = ${state.tools.textEditorVersion}',
    );
    await _saveConfig();
  }

  Future<void> setDiffTool(
    DiffToolType? tool, {
    String? path,
    String? version,
  }) async {
    Logger.debug(
      'setDiffTool called with: $tool, path: $path, version: $version',
    );
    state = state.copyWith(
      tools: state.tools.copyWith(
        diffTool: tool,
    ...
    Logger.debug(
      'After setting state, tools.diffTool = ${state.tools.diffTool}',
    );
    Logger.debug(
      'After setting state, tools.diffToolPath = ${state.tools.diffToolPath}',
    );
    Logger.debug(
      'After setting state, tools.diffToolVersion = ${state.tools.diffToolVersion}',
    );
    await _saveConfig();
  }

  Future<void> setMergeTool(
    DiffToolType? tool, {
    String? path,
    String? version,
  }) async {
    Logger.debug(
      'setMergeTool called with: $tool, path: $path, version: $version',
    );
    state = state.copyWith(
      tools: state.tools.copyWith(
        mergeTool: tool,
    ...
    Logger.debug(
      'After setting state, tools.mergeTool = ${state.tools.mergeTool}',
    );
    Logger.debug(
      'After setting state, tools.mergeToolPath = ${state.tools.mergeToolPath}',
    );
    Logger.debug(
      'After setting state, tools.mergeToolVersion = ${state.tools.mergeToolVersion}',
    );
    await _saveConfig();
  }
  ...
  Future<void> setCustomDiffToolPath(String? path, {String? version}) async {
    ...
    Logger.debug(
      'After setting state, tools.customDiffToolPath = ${state.tools.customDiffToolPath}',
    );
    Logger.debug(
      'After setting state, tools.customDiffToolVersion = ${state.tools.customDiffToolVersion}',
    );
    await _saveConfig();
  }
  ...
  Future<void> setCustomMergeToolPath(String? path, {String? version}) async {
    Logger.debug(
      'setCustomMergeToolPath called with: $path, version: $version',
    );
    // Handle explicit null (clearing) vs just updating
    if (path == null) {
      // Clearing - explicitly set both to null
    ...
    Logger.debug(
      'After setting state, tools.customMergeToolPath = ${state.tools.customMergeToolPath}',
    );
    Logger.debug(
      'After setting state, tools.customMergeToolVersion = ${state.tools.customMergeToolVersion}',
    );
    await _saveConfig();
  }
  ...
  Future<void> setNavigationRailExtended(bool extended) async {
    state = state.copyWith(
      ui: state.ui.copyWith(navigationRailExtended: extended),
    );
    await _saveConfig();
  }
  ...
  Future<void> setPreviewFontFamily(String previewFontFamily) async {
    state = state.copyWith(
      ui: state.ui.copyWith(previewFontFamily: previewFontFamily),
    );
    await _saveConfig();
  }
  ...
  Future<void> setShowHiddenFiles(bool show) async {
    state = state.copyWith(
      browse: state.browse.copyWith(showHiddenFiles: show),
    );
    await _saveConfig();
  }
  ...
  Future<void> setShowIgnoredFiles(bool show) async {
    state = state.copyWith(
      browse: state.browse.copyWith(showIgnoredFiles: show),
    );
    await _saveConfig();
  }
  ...
  Future<void> setCommandLogPanelVisible(bool visible) async {
    state = state.copyWith(
      commandLog: state.commandLog.copyWith(panelVisible: visible),
    );
    await _saveConfig();
  }
  ...
  Future<void> setCommandLogPanelWidth(double width) async {
    state = state.copyWith(
      commandLog: state.commandLog.copyWith(panelWidth: width),
    );
    await _saveConfig();
  }
  ...
  Future<void> setAutoFetch(bool enable) async {
    state = state.copyWith(
      behavior: state.behavior.copyWith(autoFetch: enable),
    );
    await _saveConfig();
  }
  ...
  Future<void> setAutoFetchInterval(int minutes) async {
    state = state.copyWith(
      behavior: state.behavior.copyWith(autoFetchInterval: minutes),
    );
    await _saveConfig();
  }
  ...
  Future<void> setConfirmPush(bool confirm) async {
    state = state.copyWith(
      behavior: state.behavior.copyWith(confirmPush: confirm),
    );
    await _saveConfig();
  }
  ...
  Future<void> addRepository(WorkspaceRepository repo) async {
    ...
    state = state.copyWith(
      workspace: state.workspace.copyWith(repositories: repos),
    );
    await _saveConfig();
  }
...
final gitConfigProvider = Provider<GitConfig>(
  (ref) => ref.watch(configProvider).git,
);
final toolsConfigProvider = Provider<ToolsConfig>(
  (ref) => ref.watch(configProvider).tools,
);
final uiConfigProvider = Provider<UiConfig>(
  (ref) => ref.watch(configProvider).ui,
);
final browseConfigProvider = Provider<BrowseConfig>(
  (ref) => ref.watch(configProvider).browse,
);
final commandLogConfigProvider = Provider<CommandLogConfig>(
  (ref) => ref.watch(configProvider).commandLog,
);
final behaviorConfigProvider = Provider<BehaviorConfig>(
  (ref) => ref.watch(configProvider).behavior,
);
final historyConfigProvider = Provider<HistoryConfig>(
  (ref) => ref.watch(configProvider).history,
);
...
// Git
final gitExecutablePathProvider = Provider<String?>(
  (ref) => ref.watch(gitConfigProvider).executablePath,
);
final defaultUserNameProvider = Provider<String?>(
  (ref) => ref.watch(gitConfigProvider).defaultUserName,
);
final defaultUserEmailProvider = Provider<String?>(
  (ref) => ref.watch(gitConfigProvider).defaultUserEmail,
);

// Tools
final preferredTextEditorProvider = Provider<String?>(
  (ref) => ref.watch(toolsConfigProvider).textEditor,
);
final preferredDiffToolProvider = Provider<DiffToolType?>(
  (ref) => ref.watch(toolsConfigProvider).diffTool,
);
final preferredMergeToolProvider = Provider<DiffToolType?>(
  (ref) => ref.watch(toolsConfigProvider).mergeTool,
);

// UI
final themeModeProvider = Provider<ThemeMode>(
  (ref) => ref.watch(uiConfigProvider).themeMode,
);
final useSystemThemeProvider = Provider<bool>(
  (ref) => ref.watch(uiConfigProvider).useSystemTheme,
);
final repositoriesViewModeProvider = Provider<RepositoriesViewMode>(
  (ref) => ref.watch(uiConfigProvider).repositoriesViewMode,
);
final projectsViewModeProvider = Provider<ProjectsViewMode>(
  (ref) => ref.watch(uiConfigProvider).projectsViewMode,
);
...
final fontFamilyProvider = Provider<String>(
  (ref) => ref.watch(uiConfigProvider).fontFamily,
);
final previewFontFamilyProvider = Provider<String>(
  (ref) => ref.watch(uiConfigProvider).previewFontFamily,
);
final fontSizeProvider = Provider<AppFontSize>(
  (ref) => ref.watch(uiConfigProvider).fontSize,
);
final previewFontSizeProvider = Provider<AppFontSize>(
  (ref) => ref.watch(uiConfigProvider).previewFontSize,
);
final localeProvider = Provider<String?>(
  (ref) => ref.watch(uiConfigProvider).locale,
);

// Browse
final showHiddenFilesProvider = Provider<bool>(
  (ref) => ref.watch(browseConfigProvider).showHiddenFiles,
);
final showIgnoredFilesProvider = Provider<bool>(
  (ref) => ref.watch(browseConfigProvider).showIgnoredFiles,
);
final browseViewModeProvider = Provider<BrowseViewMode>(
  (ref) => ref.watch(browseConfigProvider).viewMode,
);

// Command Log
final commandLogPanelVisibleProvider = Provider<bool>(
  (ref) => ref.watch(commandLogConfigProvider).panelVisible,
);
final commandLogPanelWidthProvider = Provider<double>(
  (ref) => ref.watch(commandLogConfigProvider).panelWidth,
);

// Behavior
final autoFetchProvider = Provider<bool>(
  (ref) => ref.watch(behaviorConfigProvider).autoFetch,
);
final autoFetchIntervalProvider = Provider<int>(
  (ref) => ref.watch(behaviorConfigProvider).autoFetchInterval,
);
...
// History
final defaultCommitLimitProvider = Provider<int>(
  (ref) => ref.watch(historyConfigProvider).defaultCommitLimit,
);
...
final allRequiredSettingsConfiguredProvider = Provider<bool>((ref) {
  ...
  final hasGitExecutable =
      config.git.executablePath != null &&
      config.git.executablePath!.isNotEmpty;
  final hasTextEditor =
      config.tools.textEditor != null && config.tools.textEditor!.isNotEmpty;
  final hasDiffTool = config.tools.diffTool != null;
  final hasMergeTool = config.tools.mergeTool != null;
  final hasUserName =
      config.git.defaultUserName != null &&
      config.git.defaultUserName!.isNotEmpty;
  final hasUserEmail =
      config.git.defaultUserEmail != null &&
      config.git.defaultUserEmail!.isNotEmpty;

  return hasGitExecutable &&
      hasTextEditor &&
      hasDiffTool &&
      hasMergeTool &&
      hasUserName &&
...
final missingRequiredSettingsProvider = Provider<List<RequiredSetting>>((ref) {
  ...
  if (config.git.defaultUserName == null ||
      config.git.defaultUserName!.isEmpty) {
    missing.add(RequiredSetting.userName);
  }
  if (config.git.defaultUserEmail == null ||
      config.git.defaultUserEmail!.isEmpty) {
