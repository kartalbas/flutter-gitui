...
function Get-CommitsSinceLastBuild {
    ...
    $lastBuildFile = Join-Path $ReleaseDir ".last-build-commit"
    $lastCommit = $null
...
function Update-HistoricalChangelog {
    ...
    $changelogFile = Join-Path $ProjectRoot "assets/changelog.json"
...
Write-Host "Generating AI-powered changelog..." -ForegroundColor Yellow

try {
    # Get commits since last build
    $lastTag = $Tag
    $commitData = Get-CommitsSinceLastBuild -ReleaseDir $ReleaseDir -LastTag $lastTag
    ...
    Update-HistoricalChangelog -ProjectRoot $ProjectRoot -Version $Version -CommitShort $CommitShort -ChangelogContent $changelogContent

    # Save current commit as last build
    Save-LastBuildCommit -ReleaseDir $ReleaseDir -CommitFull $CommitFull
