...
| `avoid_text_with_style` | `Text()` with a hand-set style: any `style:` that is not a `copyWith` over a `textTheme` role, and any such `copyWith` whose `color:` restates the ambient foreground (`….onSurface`). The application never owns the colour of a styled word (#432): the ramp already applies the scheme's `onSurface` to every step (`AppTheme._brightnessCorrectedTextTheme`), so a restated colour is a defect, not a licence — the inversion of this rule's first form, which *demanded* a colour mention as the price of a hand-set style. A colourless `copyWith` over a role passes (it adjusts facets the vocabulary has no word for yet, inheriting the ramp's colour), and a `copyWith` naming a colour that *differs* from ambient passes under protest as the tone backlog. The restatement half binds only the application's `lib/` — a skin is exactly where a colour decision is allowed to live (SKIN-CONTRACT.md §3.6), the same scoping as `token_read_is_mechanical`. | `BaseLabel` subclasses with `role:` and `tone:`; for a restated `….onSurface`, delete the `color:` argument — the ramp already says it |
