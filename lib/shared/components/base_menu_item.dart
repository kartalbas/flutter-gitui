...
///
/// The application used to declare its own complete sealed `MenuEntry` /
/// `MenuAction` / `MenuSeparator` / `MenuActionRole` here - written before
/// the contract existed, then duplicated by it, then kept alive beside it
/// because forty call sites imported this file. Two sealed hierarchies with
/// the same names and the same shape in one codebase is a defect with a
/// countdown on it: the day one of them grows a kind the other lacks, a
/// restatement function silently drops it. The application's copy is deleted;
/// every call site keeps this import and now speaks the contract's types,
/// which is what `BaseListItem.contextMenuItems` and `Overlays.anchor` expect
/// without restatement.
