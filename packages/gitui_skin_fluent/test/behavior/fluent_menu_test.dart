/// The point-anchored menu, measured: WinUI's context flyout from the paint
/// stream and the route's own clock, plus the four loud fences the rest of
...
void main() {
  ...
  group('the fences that remain', () {
    ...
    testWidgets('the anchored trigger refuses at build', (
      WidgetTester tester,
    ) async {
      await pumpFluentOverlayApp(
        tester,
        (BuildContext context) => Overlays.anchor(
          spec: const MenuAnchorSpec(icon: IconRole.copy, tooltip: 'More'),
          entries: <MenuEntry>[MenuAction(label: 'Copy', onPressed: () {})],
        ),
      );
...
/// Matches the [UnimplementedError] a fence throws.
const Matcher isUnimplementedError = TypeMatcher<UnimplementedError>();
